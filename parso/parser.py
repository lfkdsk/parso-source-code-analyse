...
because it splits the code into different words (tokens).

什么鬼啊 你们写个分词居然是最重要的部分啊。

Sometimes it looks a bit messy. Sorry for that! You might ask now: "Why didn't you use the ``ast``
module for this? Well, ``ast`` does a very good job understanding proper Python
code, but fails to work as soon as there's a single line of broken code.

这句话说的有道理， Broken Code 的特殊需求是有道理的，我继续看。
...
这个有点类似 Parser Combinator 的概念，针对不同的部分有不同的 Parser 进行组合处理。
...
class BaseParser(object):
    node_map = {}
    default_node = tree.Node  # 之后会有特化的节点类
    ...
    def parse(self, tokens):
        # symbol to number 说明 Symbol 有 Tag 能用来做一些便捷处理什么的
    ...
    # 覆盖的特化 Node 和 Leaf 类，和 default 的区别而已
